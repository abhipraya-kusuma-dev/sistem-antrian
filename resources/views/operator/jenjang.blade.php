...
@section('content')
    <div class="p-8">
        ...
        <nav class="flex space-x-2 mt-8">
            <a href="/operator/antrian/jenjang/{{ $jenjang }}/belum?tanggal_pendaftaran={{ $tanggal_pendaftaran }}"
                class="{{ $status === 'belum' ? 'text-blue-600 border-b-4 border-blue-600 font-bold' : 'text-blue-600/80' }}">
                Belum Terpanggil
            </a>
            <a href="/operator/antrian/jenjang/{{ $jenjang }}/sudah?tanggal_pendaftaran={{ $tanggal_pendaftaran }}"
                class="{{ $status === 'sudah' ? 'text-blue-600 font-bold border-b-4 border-blue-600' : 'text-blue-600/80' }}">
                Terpanggil
            </a>
            <a href="/operator/antrian/jenjang/{{ $jenjang }}/lewati?tanggal_pendaftaran={{ $tanggal_pendaftaran }}"
        ...
        <ul id="antrian-list" class="text-green-600 border-2 border-black grid grid-cols-4 gap-4 text-xl p-4 mt-4">
            @if (count($semua_antrian))
                @foreach ($semua_antrian as $antrian)
                    <li class="w-full">
                        <a class="bg-green-400 text-white py-1.5 inline-block text-center w-full"
                            href="/operator/antrian/panggil/{{ $antrian->id }}">
