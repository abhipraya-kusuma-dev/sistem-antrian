...
@section('content')
    <div id="daftar-container">
        ...
        @if (session('create-error'))
            <p id="message" class="fixed z-[10] rounded px-4 text-lg inset-x-4 top-4 bg-red-600 text-white font-bold py-2">
                {{ session('create-error') }}</p>
        @endif

        @if (session('create-success'))
            <p id="message"
                class="fixed z-[10] rounded px-4 text-lg inset-x-4 top-4 bg-green-600 text-white font-bold py-2">
                {{ session('create-success') }}</p>
        @endif
        ...
        <main class="flex flex-col items-center space-y-10  ">
            ...
            <div class="grid grid-cols-2 gap-4 w-full px-4" id="daftar-antrian">
                @foreach ($jenjang as $j)
                    <a href="/antrian/daftar/konfirmasi/{{ $j }}" id="daftar-link"
                        class="flex w-full justify-between rounded-md items-center p-8  text-white uppercase"
                        style="background-color: {{ $warna[$loop->index] }};">
                        <span class="text-2xl font-bold">{{ $j }}</span>
                        <span class="text-xl text-stroke-0 font-bold"><span>Sisa antrean:
                            </span>{{ count($antrian[$j]) }}</span>
                    </a>
                @endforeach
